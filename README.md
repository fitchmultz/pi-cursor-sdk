...
# Keep a pending live run for 15 minutes before idle cleanup (default 5 minutes).
PI_CURSOR_LIVE_RUN_IDLE_DISPOSE_MS=900000 pi --model cursor/grok-4.6
...
`PI_CURSOR_LIVE_RUN_IDLE_DISPOSE_MS` sets the idle cleanup deadline for pending local live runs in milliseconds. The default is 300000 ms. Use a base-10 integer from 1 to 2147483647; missing or invalid values use the default. This deadline is separate from the MCP and pi bridge call timeouts. A longer delay postpones cleanup but does not prevent it from cancelling an active bridged tool when the timer expires.
