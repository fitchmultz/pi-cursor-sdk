...
```json
{
  ...
  "local": {
    ...
    "resume": true,
    "storeRoot": "/var/tmp/alice/pi-cursor-sdk"
  }
}
```

`local.storeRoot` is an optional user/environment setting only (`PI_CURSOR_SDK_STATE_ROOT` or user config). When unset, pi keeps the SDK default workspace store under `~/.cursor/projects/.../sdk-agent-store/...`. When set, it relocates pi-cursor-sdk SQLite (`index.db` and WAL) under `<storeRoot>/<sha256(resolve(cwd))[0:32]>/` instead; it does not move `~/.cursor` authentication or model cache. Set it to any absolute directory (e.g. `/var/tmp/<user>/pi-cursor-sdk` on Unix or `%LOCALAPPDATA%/pi-cursor-sdk` on Windows); JSON config does not expand `$USER`. On Unix the custom root is created mode `0700` only when owned by the current user; on Windows it must be a writable non-symlink directory. If a custom root is not writable or violates ownership checks, the extension fails closed instead of falling back to `~/.cursor`. There is no `--cursor-store-root` flag.
