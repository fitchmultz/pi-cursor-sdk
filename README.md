...
# Ask Cursor to stop branding commits/PRs with Co-authored-by trailers.
PI_CURSOR_ATTRIBUTION=0 pi --model cursor/grok-4.6
# Or toggle it inside an interactive Cursor session.
/cursor-attribution off
...
`PI_CURSOR_ATTRIBUTION=0` turns off Cursor commit/PR attribution. `/cursor-attribution on`, `/cursor-attribution off`, and `/cursor-attribution toggle` set the branch-scoped session preference and save the user default as `local.attributeCommitsToAgent` in `~/.pi/agent/cursor-sdk.json`; `/cursor-attribution` with no argument reports the effective state. Precedence is session command/history, `PI_CURSOR_ATTRIBUTION`, trusted project config, user config, then the built-in default of attribution on (matching the Cursor SDK). When disabled, the footer shows `attrib:off` and the prompt tells Cursor not to add `Co-authored-by: Cursor` trailers or any other Cursor attribution to commits, PRs, or comments. This is prompt-level only: the installed `@cursor/sdk` defaults `attributeCommitsToAgent` on for embedders, exposes no public option to disable it, and does not read `~/.cursor/cli-config.json`, so a stray trailer is still possible and must be amended.
