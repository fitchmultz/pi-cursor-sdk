...
- **AGENTS.md is not duplicated on Cursor models when Cursor loads the same rules.** Pi still discovers global and project `AGENTS.md` files unless you start with `-nc`, but on `cursor/*` models the extension removes only the matching `<project_instructions>` blocks that overlap Cursor `settingSources` (`user` for `~/.pi/agent/AGENTS.md`, `project` for repo files, or both/`all`). With `PI_CURSOR_SETTING_SOURCES=none` or `plugins`-only, pi context is left intact. Switching to a non-Cursor model restores pi's full block on the next message. Set `PI_CURSOR_PRESERVE_PI_AGENTS_MD=1` to keep the old duplicate behavior.
