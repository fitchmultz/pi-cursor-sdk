...
- **AGENTS.md is not duplicated on Cursor models.** Pi still discovers global and project `AGENTS.md` files unless you start with `-nc`, but on `cursor/*` models the extension strips pi's `<project_context>` block from the Cursor-facing prompt so Cursor `settingSources` remain the single loader. Switching to a non-Cursor model restores pi's full block on the next message. Set `PI_CURSOR_PRESERVE_PI_AGENTS_MD=1` to keep the old duplicate behavior.
