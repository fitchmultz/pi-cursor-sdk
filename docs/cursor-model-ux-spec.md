...
- Exact `@cursor/sdk@1.0.27` is a package dependency of this extension; users should not need a global SDK install. Pi 0.84.0 remains the minimum supported version; Pi 0.87.0 is the current compatibility qualification baseline, while optional published Pi core peer dependencies use `"*"` ranges per current Pi package guidance.
