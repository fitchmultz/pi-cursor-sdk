...
- On `cursor/*` models, pi-cursor-sdk removes only pi-generated `<project_instructions>` blocks that overlap the effective Cursor `settingSources` (`user` for global `~/.pi/agent/AGENTS.md`, `project` for discovered repo files, or both when `all` / `project,user`). Removal uses exact `contextFiles` payloads from `before_agent_start`, not regex parsing of free-form prompt text. Suppression is skipped when pi started with `-nc` (no loaded context files), when `PI_CURSOR_SETTING_SOURCES=none`, when narrowed sources such as `plugins` do not load the matching layer, or when `PI_CURSOR_PRESERVE_PI_AGENTS_MD=1`. Switching away from a Cursor model restores pi's full context block on the next user message.
