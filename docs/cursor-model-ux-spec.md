...
- Cursor-side thinking remains visible through pi's native thinking rendering when the Cursor SDK emits thinking or summary deltas. Cursor models use Cursor SDK local-agent tools and configured Cursor MCP/plugin tools, not pi's native provider tool schemas or third-party pi extension tools. Cursor internal tool activity is recorded from SDK events and scrubbed. In interactive TTY sessions, supported completed `read`, `bash`, `grep`, `find`, `ls`, `edit`, `write`, `readLints`, `delete`, `updateTodos`, `createPlan`, `task`, `generateImage`, and `mcp` activity is replayed through pi's native tool-call rendering path with recorded Cursor results, so the TUI can show native cards without forcing Cursor to call pi tools or rerunning Cursor's reads/shell commands/file edits. Cursor `glob` activity is replayed through native `find` cards. Cursor edit/write activity is replayed through `cursor_edit` and `cursor_write` cards rather than pi's built-in `edit`/`write` names because Cursor's edit/write schemas differ from pi's schemas; Cursor lints, delete, todos/plans, task, image, and MCP activity use replay-only cards such as `cursor_read_lints`, `cursor_delete`, and `cursor_task`. These replay-only tools display recorded Cursor results, normalize workspace-local paths/diff headers for display, and fail closed if called without a recorded result. Native replay wrappers are registered only for tool names not already owned by another extension; conflicting tools use the bounded scrubbed transcript fallback. Cursor workflow tools such as `SwitchMode` and Cursor todo state are not pi workflow controls; reported todo/plan events are displayed as Cursor activity only. Started Cursor SDK tool calls that never receive a completion event can be surfaced as error replay cards when a wrapper exists. `PI_CURSOR_NATIVE_TOOL_DISPLAY=0` disables native replay, and `PI_CURSOR_REGISTER_NATIVE_TOOLS=0` is a registration-only opt-out that keeps the transcript fallback without shadowing pi tool names. When these native cards are emitted, the provider mirrors Codex's turn shape as Cursor SDK completions arrive: assistant `toolUse`, pi `toolResult`s, live post-tool Cursor thinking/text, any later Cursor tool batches as further `toolUse` turns, then Cursor's final assistant answer. Non-interactive runs keep bounded scrubbed transcript output instead, preserving `pi -p` assistant text output. Cursor text deltas stream live when native tool replay is not active.
...
- Audit observation, 2026-05-19: a missing-file read with Composer 2.5 emitted `tool-call-started` for Cursor `read`, then streamed final text `Error: File not found`, but did not emit `tool-call-completed` or an `onStep` `toolCall` error result. The replay path now flushes leftover started calls at run completion as error replay cards when a wrapper exists, or as transcript notes otherwise.
