...
- Audit observation, 2026-05-19: a missing-file read with Composer 2.5 emitted `tool-call-started` for Cursor `read`, then streamed final text `Error: File not found`, but did not emit `tool-call-completed` or an `onStep` `toolCall` error result. The current replay path renders only completed Cursor tool calls, so this orphaned start is not shown as a native red `read` card. Track this as a Cursor SDK/provider UX gap before changing replay semantics for incomplete tool calls.
