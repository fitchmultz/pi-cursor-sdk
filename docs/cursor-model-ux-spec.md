...
- After each finished SDK run, the provider calls `agent.getUsage()` (no `runId` on local agents; `runId` only for cloud `run-*` IDs) and maps billed spend into pi `usage` spend fields. Occupancy `totalTokens` uses only in-window local turn-ended occupancy; billed rows never become occupancy. Fallback floors ignore assistant measurements timestamped at or before the latest compaction summary, including retained messages placed after that summary and summaries converted to user messages by Pi.
