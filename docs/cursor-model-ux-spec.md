...
- On `cursor/*` models, pi-cursor-sdk suppresses pi's `<project_context>` / `AGENTS.md` injection (global `~/.pi/agent/AGENTS.md` plus discovered project files) from the Cursor-facing system prompt so instructions are not duplicated with Cursor `settingSources`. Suppression applies on each turn via `before_agent_start` and again in `sanitizeSystemPromptForCursor()`. It is skipped when pi started with `-nc` / `--no-context-files` (no loaded context files) or when `PI_CURSOR_PRESERVE_PI_AGENTS_MD=1`. Switching away from a Cursor model restores pi's full context block on the next user message; switching to Cursor strips it again. `PI_CURSOR_SETTING_SOURCES` is unchanged.
