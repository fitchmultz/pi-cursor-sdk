...
- Session-scoped Cursor SDK agent pooling reuses one live `@cursor/sdk` agent across compatible follow-up turns within the same pi session scope. Ready pooled agents that sit idle for 10 minutes or longer are disposed at the next acquire so the turn creates or resumes a fresh SDK handle before `send` (observed post-idle `ConnectError` / `[unauthenticated]` floor is ~13 minutes). If a reused pooled agent still fails with unauthenticated auth before any assistant progress, the provider one-shot resets and retries the turn with a fresh agent; exhausted recovery uses distinct idle-reconnect copy instead of blaming a missing API key. `planCursorSessionSend()` in `src/cursor-session-send-policy.ts` decides whether the next turn sends a full bootstrap prompt or an incremental follow-up, whether the SDK agent must be recreated, and why. `computeCursorContextFingerprint()` and `shouldBootstrapCursorContext()` remain the context-only bootstrap signal. The pool recreates the agent when context diverges, when branch or compaction summaries appear after `/tree` navigation or compaction, after 20 completed incremental sends, when the API key identity changes, after send errors, on `session_shutdown`, and when `session_before_tree` / `session_tree` invalidate the active branch. Incremental sends omit the full Cursor SDK tool boundary block because the session agent retains prior bootstrap context, but every send ends with a short tool tail guard placed after the latest user request (including an explicit shell `cd` hint).
