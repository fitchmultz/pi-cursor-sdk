...
- Persisted pi sessions use a session-scoped Cursor SDK SQLite store at `<workspace-state-root>/pi-sessions/<session-hash>/` (`~/.cursor/projects/.../sdk-agent-store/...` by default, or under custom `local.storeRoot` / `PI_CURSOR_SDK_STATE_ROOT`); create/resume, transcript reads, checkpoint lookup, and exact-ID cleanup all receive that same store. Custom roots do not relocate Cursor IDE or `~/.cursor` auth and fail closed on invalid/unowned permissions. Fileless acquisitions use unique OS-temporary stores that are removed on graceful disposal; invalidation starts a fresh agent instead of reopening a disposed temporary store. Resume entries version the store identity. A recorded identity from a different custom `storeRoot` takes resume-fallback. Removing a persisted pi session does not automatically remove its store directory; only a verified session-derived `pi-sessions/<session-hash>` root may be removed after no pi process uses it. Cloud agents are unchanged.
