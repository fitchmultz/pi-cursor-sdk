...
- After each finished SDK run, the provider calls `agent.getUsage()` (no `runId` on local agents; `runId` only for cloud `run-*` IDs) and maps billed spend into pi `usage`. Occupancy `totalTokens` still prefers in-window local turn-ended occupancy so compaction is not poisoned by cumulative billed input.
