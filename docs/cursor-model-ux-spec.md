...
- Commit attribution defaults on to match the Cursor SDK and is opted out through `PI_CURSOR_ATTRIBUTION=0`, `/cursor-attribution on|off|toggle`, or `cursor-sdk.json` `local.attributeCommitsToAgent`. Precedence is session, environment, trusted project, user, then the built-in on default. Suppression is prompt-level: bootstrap and incremental prompts ban `Co-authored-by: Cursor` trailers and other Cursor attribution, because the installed SDK exposes no attribution option to embedders and ignores `~/.cursor/cli-config.json`. The footer adds `attrib:off` when disabled.
...
| Set commit attribution | `/cursor-attribution on\|off\|toggle` | Cursor model selected |
