...
- Native read replay uses pi `DEFAULT_MAX_LINES` / `DEFAULT_MAX_BYTES` truncation so green read cards match built-in `read`. Cursor `thinking-delta` and `text-delta` events queue into the same native replay assistant turn, including pre-tool content when the SDK streams it. `cursor_edit` / `cursor_write` replay cards use native-style edit/write titles.
