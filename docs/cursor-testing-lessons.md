...
**Prompt-format mitigation (0.1.63+):** `buildCursorPrompt()` now serializes executed history as `[prior-tool …]` / `[prior-tool-result …]` (not mimickable `Tool call (` cards), scrubs narrated `Tool call(` / `CallMcpTool(` lines from assistant text blocks, strengthens the tool tail guard, and appends an incremental recovery instruction when the latest user message still contains narrated tool text. Residual #40 reports after that version still need JSONL to distinguish model text echo from #55/#52/#43.
