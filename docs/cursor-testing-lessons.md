...
**What the screenshot in [#40](https://github.com/fitchmultz/pi-cursor-sdk/issues/40) shows:** Plain assistant text that mirrors an older pi **prompt transcript format** for replay tool calls (`Tool call (Cursor activity, call cursor-replay-…): …`) rather than a rendered pi `toolCall` card. That pattern usually means the Cursor model **narrated** a tool call as text; it is not proof that pi failed to emit `toolcall_start` / `toolUse`.

**Prompt-format mitigation (0.1.63+):** `buildCursorPrompt()` now serializes executed history as `[prior-tool …]` / `[prior-tool-result …]` (not mimickable `Tool call (` cards), scrubs narrated tool-invocation spans from assistant text with a balanced-paren scanner (handles wrapped multi-line and prefixed cards), strengthens the tool tail guard, and appends an incremental recovery instruction when the latest user message still contains narrated tool text.

**Ledger cross-check + in-turn repair (0.1.63+):** Narration is only asserted when scanned tool-invocation text appears **and** the turn's `CursorToolCompletionLedger` reports zero completed tools. Finished turns that match get **one** bounded corrective continuation (never loops; skipped on cancel/error/abort). Kill switch: `PI_CURSOR_NARRATED_TOOL_REPAIR=0`. With `PI_CURSOR_SDK_EVENT_DEBUG=1`, repairs are recorded in `coordinator-events.jsonl` under phase `narrated-tool-turn-repair`. Residual #40 reports after that version still need JSONL to distinguish model text echo from #55/#52/#43.
...
3. **`content` is only `type: "text"` and text contains `Tool call (` / `CallMcpTool(` / `cursor-replay-` / serialized arg keys** — model text echo of a mimickable transcript format; not #55, not #52 stale routing. Compare with `buildCursorPrompt()` output in the prior turn (0.1.63+ uses `[prior-tool …]` and should scrub those narrated lines from assistant history).
